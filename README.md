...
- Node.js 環境でグローバル `WebSocket` が無い場合は、`webSocketFactory` を渡してください。
...
console.log(ticker.data[0]);

const margin = await client.getMargin();
console.log(margin.data.availableAmount);
```
...
const ws = client.connectPrivateWebSocket(token.data);
...
```bash
node --experimental-strip-types --eval "import('./src/index.ts')"
```
